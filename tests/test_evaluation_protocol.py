...
def test_freeze_records_hashes_exposure_and_legacy_overlap(tmp_path):
    development = _write(
        ...
        [_case("dev-1", "Cir_2024_01_fr.pdf")],
    )
    validation = _write(
        tmp_path / "validation.json",
        [_case("val-1", "Cir_2024_01_fr.pdf")],
    )
    holdout = _write(
        tmp_path / "holdout.json",
        [_case("holdout-1", "Note_2026_02_ar.pdf", language="ar")],
    )
    ...
    assert protocol["sets"]["final_holdout"]["languages"] == {"ar": 1}
...
def test_freeze_rejects_validation_holdout_family_leakage(tmp_path):
    development = _write(
        tmp_path / "development.json",
        [_case("dev-1", "Cir_2024_09_fr.pdf")],
    )
    validation = _write(
        tmp_path / "validation.json",
        [_case("val-1", "Note_2023_07_ar.pdf", language="ar")],
    )
    holdout = _write(
        tmp_path / "holdout.json",
        [_case("holdout-1", "Note_2026_07_ar.pdf", language="ar")],
    )
...
def test_freeze_parses_compact_circular_names_and_fails_closed_on_unknown_sources(tmp_path):
    ...
    validation = _write(
        tmp_path / "validation.json",
        [_case("val-1", "Cir202204fr.pdf")],
    )
    overlapping_holdout = _write(
        tmp_path / "overlapping-holdout.json",
        [_case("holdout-1", "Cir_2026_04_fr.pdf")],
    )
...
def test_freeze_rejects_development_holdout_family_leakage_by_default(tmp_path):
    development = _write(
        tmp_path / "development.json",
        [_case("dev-1", "Note_2023_07_ar.pdf", language="ar")],
    )
    validation = _write(
        tmp_path / "validation.json",
        [_case("val-1", "Cir_2024_01_fr.pdf")],
...
def test_freeze_treats_french_and_arabic_editions_as_one_family(tmp_path):
    ...
    validation = _write(
        tmp_path / "validation.json",
        [_case("val-1", "Note_2023_07_fr.pdf")],
    )
    holdout = _write(
        tmp_path / "holdout.json",
        [_case("holdout-1", "Note_2026_07_ar.pdf", language="ar")],
...
def test_freeze_rejects_family_override_for_recognized_source(tmp_path):
    ...
    validation = _write(tmp_path / "validation.json", [validation_case])
    holdout = _write(
        tmp_path / "holdout.json",
        [_case("holdout-1", "Note_2026_08_ar.pdf", language="ar")],
    )
    ...
    unknown_holdout = _write(
        ...
        [_case("holdout-2", "Decision_alpha_ar.pdf", language="ar")],
    )
    with pytest.raises(ValueError, match="document_family"):
        freeze_protocol(
            ...
            validation=_write(
                ...
                [_case("val-2", "Note_2023_07_ar.pdf", language="ar")],
            ),
