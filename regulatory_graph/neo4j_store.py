...
class Neo4jRegulatoryGraph:
    ...
    def affected_provisions(
        ...
        *,
        ...
        result = self._execute(
            ...
            "(target_version:ProvisionVersion)<-[:HAS_VERSION]-"
            "(span_provision:Provision) "
            "WHERE type(within) = 'WITHIN' "
            "AND target_version.verification_status = 'VERIFIED' "
    ...
    def lineage(self, provision_uid: str) -> tuple[LineageEntry, ...]:
        result = self._execute(
            ...
            "(target_version:ProvisionVersion)<-[:HAS_VERSION]-(target) "
            "WHERE type(within) = 'WITHIN' "
            "AND target_version.verification_status = 'VERIFIED' }) "
            "OPTIONAL MATCH (event)-[:INTRODUCES_VERSION]->(introduced:ProvisionVersion) "
            "WHERE introduced.verification_status = 'VERIFIED' "
            "AND introduced.provision_uid = target.uid "
            "WITH event, source, target, "
            "collect(DISTINCT introduced.uid) AS introduced_uids "
            "OPTIONAL MATCH (event)-[retired_relation]->(retired:ProvisionVersion) "
            "WHERE type(retired_relation) = 'RETIRES_VERSION' "
            "AND retired.verification_status = 'VERIFIED' "
            "AND retired.provision_uid = target.uid "
